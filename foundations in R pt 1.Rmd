...
```{r}
print(rep(seq(from =1, to =20, by =3), times=3))
...
```{r}
cookieData <- cbind.data.frame(Animal, Cookies)
print(cookieData)
```
...
```{r}
TVShows <- c("XFiles", "Millenium", "Pokemon", "DoctorWho")
...
```{r}
Favorites <- cbind.data.frame(TVShows, TVRating)
```
